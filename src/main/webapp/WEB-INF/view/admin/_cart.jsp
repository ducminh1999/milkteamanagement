...
<body onload="updatePrice()">
										...
										<td>
											<div class="carttable_product_item">
												<div class="item_image">
													<a href="<%=contextPath%>/product?id=<%=listCart.get(i).getProductId()%>"><img
														src="<%=listProduct.get(i).getImageFile()%>" alt="image_not_found"></a>
												</div>
												...
												<h3 class="item_title"><%=listProduct.get(i).getName()%></h3>
											</div>
										</td>
										<td><span class="price_text1"><%=listProduct.get(i).getPrice()%> VND</span></td>
										<td>
											<div class="quantity_input">
												<input class="input_number" name="quantity" id="sl<%=listProduct.get(i).getId()%>" onkeyup="updatePrice();"
													type="text" min="0" max="30" value="<%=listCart.get(i).getQuantity()%>">
											</div>
										</td>
										<td><span class="price_text2" id="total<%=listProduct.get(i).getId()%>"></span></td>
									</tr>
								</form>
								<%
									}
	...
	<script>
		function updatePrice() {
			var sum = 0;
			<%for (int i = 0; i < listCart.size(); i++) {%>
				var sl = document.getElementById("sl"+"<%=listProduct.get(i).getId()%>").value;
			var price =
